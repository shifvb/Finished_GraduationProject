...
def classification_service(train_folder: str, test_folder: str):
    return _train(train_folder, test_folder)


def _gen_batches(images: np.ndarray, labels: np.ndarray, examples_list: list):
    """
...
def _show_num_each_class(labels: np.ndarray, msg: str) -> None:
...
def _train(train_folder: str, test_folder: str):
    # 加载训练集文件
    images, labels = persistents.classification_persistent(train_folder)
    _show_num_each_class(labels, "训练集输入构成")
    # 生成训练集
    train_images, train_labels = _gen_batches(images, labels, [459, 106, 117, 192, 0, 459])
    _show_num_each_class(train_labels, "生成训练集构成")
    # 加载测试集文件
    images, labels = persistents.classification_persistent(test_folder)
    _show_num_each_class(labels, "测试集构成")
    # 生成测试集
    test_images, test_labels = _gen_batches(images, labels, [-1, 0, 0, 0, 0, 0])
    _show_num_each_class(test_labels, "生成测试集构成")
    # 构建稀疏自编码器
    ...
    h3 = tf.nn.softplus(tf.matmul(h2, w3) + b3)
    y = h3
    # 定义loss, train_step, correct_prediction, accuracy
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            if i % 10 == 0:
                ...
                accuracy_max = max(accuracy_max, test_accuracy)
